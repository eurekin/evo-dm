...
//------------------------------------------------------------------------------
    ...
    public boolean hasClass(int class_id) {
        for (int c = 0; c < ClassesNames.size(); c++) {
            if (class_id == ClassesNames.get(c)) {
                return true;
            }
        }
        return false;
...
//------------------------------------------------------------------------------
    ...
    public int getClassNameNext() {
        if (currentClass >= ClassesNames.size() - 1) {
            return -1;
        }
        currentClass++;
        return ClassesNames.get(currentClass);
    ...
    public boolean isSatisfy(Condition C) {
        ...
        for (int s = 0; s < Segments; s++) {
            switch (C.getRelation()) {
                case IN:
                    if (this.values[s][C.getAttrib()] >= C.getValue1() && this.values[s][C.getAttrib()] <= C.getValue2()) {
                        return true;
                    }
...
//------------------------------------------------------------------------------
    ...
    public float getMaxAttribValue(int attrib) {
        ...
        for (int s = 0; s < Segments; s++) {
            if (this.values[s][attrib] > max) {
                max = this.values[s][attrib];
            }
        }
        return max;
    }
...
//------------------------------------------------------------------------------
    ...
    public float getMinAttribValue(int attrib) {
        ...
        for (int s = 0; s < Segments; s++) {
            if (this.values[s][attrib] < min) {
                min = this.values[s][attrib];
            }
        }
        return min;
    }
