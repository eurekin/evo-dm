...
// #[regen=yes,id=DCE.A8CA3732-61C1-943F-2482-22A9B3EB11D3]
// </editor-fold> 
...
*/
    ...
    public Individual Mutation(){
    	...
    	if (Configuration.getConfiguration().getMutationType() == MutationType.FAM) 	return MutationFAM(); 
    	else return MutationSimple();
    }
   ...
   //------------------------------------------------------------------------------
    ...
    public int getClassID(){
        ...
        /*DOMYSLNA KLASA 0*/
        if (class_id > Configuration.getClassesNo()) return 0; 
    ...
    // <editor-fold defaultstate="collapsed" desc=" UML Marker "> 
    // #[regen=yes,id=DCE.26C45447-DC44-05D7-34EF-26AEC43FC2F0]
    // </editor-fold>
    /**
     ...
     *
     */
    public Individual Crossover (Individual Indv1, Individual Indv2 ) {
        Rule tym = new Rule();
        int cut = Rand.GetRandomInt(Configuration.getConfiguration().getNumberOfAttributes());
        for (int i=0;i<Configuration.getConfiguration().getNumberOfAttributes();i++){
            RuleGene d=null;
            if (i<=cut) d = new RuleGene( ((Rule)Indv1).getGene(i) ) ;
            else d = new RuleGene( ((Rule)Indv2).getGene(i) );
            tym.setGene(i, d);
        }
...
*/
        ...
        boolean classFromOne = Rand.GetRandomBoolean();
        Rule R = null;
        if (classFromOne) R = new Rule((Rule)Indv1);
        else R = new Rule((Rule)Indv2);
    ...
    public boolean isEmpty(){
        for (int attribID=0;attribID<Configuration.getConfiguration().getNumberOfAttributes();attribID++)  {
         if (this.ConditionOnAttribute.get(attribID).isOff()==false) return false;
            }
        return true;
    }
    ...
    else throw new Exception("Illegal Object!");
    return diff;	
    }
